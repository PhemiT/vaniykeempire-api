...
async function resolveThumbnail(files, fromDisk = false) {
  if (!files?.thumbnail?.[0])
    return { thumbnailUrl: null, thumbnailPublicId: null };
...
async function triggerPreviewTranscode(contentId, r2Key) {
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(
      `GitHub Actions dispatch failed (preview): ${resp.status} ${text}`
    );
  }
}
...
exports.createContentDirect = async (req, res) => {
  try {
    ...
    const { thumbnailUrl, thumbnailPublicId } = await resolveThumbnail(
      req.files,
      false
    );
...
exports.createContent = async (req, res) => {
  try {
    const { title, description, type, category, price, status, tags } =
      req.body;
    ...
    if (isVideo) {
      ...
      if (!r2Key)
        return res
          .status(400)
          .json({ error: 'r2Key is required for video uploads' });
      ...
      return res
        .status(201)
        .json({ message: 'Video queued for processing', content });
    }
    ...
    if (content.type === 'audio' && content.fileUrl) {
      content.previewUrl = content.fileUrl.replace(
        '/upload/',
        '/upload/eo_60/'
      );
      await content.save();
...
exports.updateContent = async (req, res) => {
  try {
    ...
    const isVideo =
      updates.type === 'video' || req.route?.path?.includes('/video/');
    ...
    if (req.files?.file || (isVideo && updates.r2Key)) {
      if (isVideo) {
        ...
        if (!r2Key)
          return res
            .status(400)
            .json({ error: 'r2Key is required for video replacement' });
        ...
        await content.populate('createdBy', 'name email');
        return res.json({
          message: 'Video replaced and queued for processing',
          content,
        });
      }
  ...
  try {
    ...
    if (content.type === 'video') {
      ...
      if (content.hlsMasterUrl) {
        console.log(
          'HLS segments to clean from R2 prefix:',
          content.hlsMasterUrl.replace('master.m3u8', '')
        );
      }
    } else {
      if (content.filePublicId) {
...
exports.transcodeComplete = async (req, res) => {
  try {
    ...
    if (!contentId || !hlsKey) {
      return res
        .status(400)
        .json({ error: 'contentId and hlsKey are required' });
    }
...
exports.accessContent = async (req, res) => {
  try {
    ...
    if (!purchase) {
      return res
        .status(403)
        .json({ error: 'You need to purchase this content to access it' });
    }
  ...
  try {
    ...
    const purchases = await Purchase.find({ user: userId, status: 'completed' })
      .populate({
        path: 'content',
        select:
          'title description type category price thumbnailUrl tags createdBy fileSize duration fileUrl status',
      })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort({ purchasedAt: -1 });
...
exports.setViews = async (req, res) => {
  try {
    ...
    if (typeof views !== 'number' || views < 0) {
      return res
        .status(400)
        .json({ error: 'views must be a non-negative number' });
    }
...
exports.claimFree = async (req, res) => {
  try {
    ...
    if (!content) return res.status(404).json({ error: 'Content not found' });
    if (content.price !== 0)
      return res.status(400).json({ error: 'Content is not free' });
    ...
    if (existing)
      return res.json({ message: 'Already in library', purchase: existing });
...
exports.getPreview = async (req, res) => {
  try {
    ...
    if (content.type !== 'video') {
      return res
        .status(400)
        .json({ error: 'Preview only available for video content' });
    }
...
exports.createAlbum = async (req, res) => {
  try {
    const { title, description, category, price, status, tags, tracks } =
      req.body;
...
exports.updateAlbum = async (req, res) => {
  try {
    ...
    if (content.type !== 'album')
      return res.status(400).json({ error: 'Content is not an album' });

    let removed = [];
    try {
      removed = removedTrackIds ? JSON.parse(removedTrackIds) : [];
    } catch {
      removed = [];
    }
...
exports.getPreviewUploadUrl = async (req, res) => {
  try {
    ...
    if (content.type !== 'video')
      return res
        .status(400)
        .json({ error: 'Preview upload only valid for video content' });
...
exports.previewTranscodeComplete = async (req, res) => {
  try {
    ...
    if (!contentId || !previewHlsKey)
      return res
        .status(400)
        .json({ error: 'contentId and previewHlsKey are required' });
